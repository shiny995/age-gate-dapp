...
function index () {
    return {
        ...
        visitor: {
            ...
            Program(path, state) {
                var _a;
                const { module: target = 'CommonJS' } = (_a = state.opts) !== null && _a !== void 0 ? _a : {};
                ...
                const dirnameMetas = [];
                const resolveMetas = [];
                const urlScopeIdentifiers = new Set();
                const resolveScopeIdentifiers = new Set();
                ...
                if ((urlMetas.length !== 0) || (resolveMetas.length !== 0)) {
                    ...
                    let metaUrlReplacement;
                    // eslint-disable-next-line @typescript-eslint/init-declarations -- no obvious default
                    let metaResolveReplacement;
                    switch (target) {
                        case 'CommonJS': {
                            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- deterministic casting
                            metaUrlReplacement = template.smart.ast `require('url').pathToFileURL(__filename).toString()`;
                            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- deterministic casting
                            metaResolveReplacement = (args) => template.smart.ast `require('url').pathToFileURL(require.resolve(${args})).toString()`;
                            break;
                        }
                        case 'ES6': {
                            let urlId = 'url';
                            ...
                            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- deterministic casting
                            path.node.body.unshift(template.smart.ast `import ${urlId} from 'url';`);
                            if (resolveMetas.length !== 0) {
                                // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- deterministic casting
                                path.node.body.unshift(template.smart.ast `import { createRequire as ${createRequireId} } from 'module';`);
                            }
                            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- deterministic casting
                            metaUrlReplacement = template.smart.ast `${urlId}.pathToFileURL(__filename).toString()`;
                            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- deterministic casting
                            metaResolveReplacement = (args) => template.smart.ast `${urlId}.pathToFileURL(${createRequireId}(${urlId}.pathToFileURL(__filename).toString()).resolve(${args})).toString()`;
                            break;
                        }
                    }
                ...
                // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- deterministic casting
                const metaFilenameReplacement = template.smart.ast `__filename`;
                // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- deterministic casting
                const metaDirnameReplacement = template.smart.ast `__dirname`;
